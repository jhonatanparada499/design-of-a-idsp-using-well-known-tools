...
**Abstract**  
Intrusion Detection Systems (IDS) can be defined as the act of detecting actions that attempt to compromise the confidentiality, integrity, or availability of a resource. Intrusion Detection and Prevention Systems (IDPS) follow the same process of gathering and identifying data and behavior, with the added ability to block the activity. With organizations facing over 1,200 attempted intrusions monthly and more than 1,600 confirmed data breaches recorded globally in 2024, the need for real-time IDPSs has become critical.
The purpose of this research is to design and evaluate a real-time network IDPS using open-source tools. The first part compares two open-source tools: Snort 3 (using Talos LightSPD ruleset) and Suricata (using the Emerging Threats Open ruleset). Both tools are evaluated by replaying real-world malware traffic, specifically a Lumma Stealer capture, to measure their detection performance. Suricata achieved a precision of 65%, recall of 100%, and F1 score of 78%, while Snort 3 achieved a precision of 33%, recall of 100%, and F1 score of 50%, with Suricata generating fewer false positives. The second part integrates the selected open-source tools into a real-time IDPS, including EveBox as a web-based alert management interface. Performance overhead was also evaluated, showing that a Docker-based deployment reduces memory usage by approximately 57% compared to a full QEMU/KVM virtual machine setup.

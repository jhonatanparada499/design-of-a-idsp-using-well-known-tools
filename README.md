...
**Abstract**  
Intrusion Detection Systems (IDS) can be defined as the act of detecting actions that attempt to compromise the confidentiality, integrity, or availability of a resource. Intrusion Detection and Prevention Systems (IDPS) follow the same process of gathering and identifying data and behavior, with the added ability to block the activity. With organizations facing over 1,200 attempted intrusions monthly and more than 1,600 confirmed data breaches recorded globally in 2024 alone, the need for automated, real-time prevention systems is critical.
The purpose of this research is to design and evaluate a real-time network IDPS using open-source tools as part of the CRSP CUNY research program. The first part explores and compares two leading open-source tools — Snort 3 (using Talos LightSPD rules) and Suricata (using the Emerging Threats Open ruleset) — by replaying real-world malware traffic, specifically a Lumma Stealer capture, through both engines and measuring their detection performance. Suricata achieved a precision of 65%, recall of 100%, and F1 score of 78%, while Snort 3 achieved a precision of 33%, recall of 100%, and F1 score of 50%, with Suricata generating significantly fewer false positives. The second part integrates the selected tools into a functional real-time IDPS, incorporating rule management, alert tuning to minimize false alarms, and EveBox as a web-based alert management interface. Performance overhead was also evaluated, demonstrating that a Docker-based deployment reduces memory usage by approximately 57% compared to a full QEMU/KVM virtual machine setup.
